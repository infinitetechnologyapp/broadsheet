...
window.showSection = id => {
  ...
  // NOTE: section-settings is intentionally NOT here — all roles need it
  // (Theme changer + Change Password are relevant to every user)
  const adminOnly = ["section-recordbank","section-approval",
...
function applyRoleUI() {
  ...
  setDisplayFlex("nav-remarks",        _isFT);                        // 7b. FT only
...
// Load class subjects into checkboxes
// Searches ALL 3 terms and MERGES results so new subjects added in any term appear
async function loadSubjectCheckboxes(classBase, selected) {
  ...
  // Try current session across all 3 terms and MERGE — never stop at first result
  // This ensures newly added subjects in any term are always visible
  const allSubjectSet = new Set();

  for (const t of ["1","2","3"]) {
    try {
      const subs = await getClassSubjects(classBase, t, _currentSession);
      subs.forEach(s => allSubjectSet.add(s));
    } catch(e) { /* term may not exist yet */ }
  }

  // Also try without session tag (legacy subjects stored without session)
  try {
    const legacySubs = await getClassSubjects(classBase, "1", "");
    legacySubs.forEach(s => allSubjectSet.add(s));
  } catch(e) { /* ignore */ }

  const subjects = Array.from(allSubjectSet).sort();

  if (!subjects.length) {
    wrap.innerHTML = `<p style="color:var(--warning);font-size:.82rem">
      ⚠ No subjects found for <strong>${classBase}</strong>.<br>
      Go to the <strong>Subjects</strong> section, select this class and term, add your subjects, then click Save.</p>`;
    return;
  }
...
window.editStudent = async function(reg) {
  ...
  // Always load subject checkboxes when editing a student
  // Previously only loaded if isAll was false — this meant newly added subjects
  // were invisible when a student was set to "all subjects"
  if (s.classBase) {
    const selectedSubs = Array.isArray(s.subjectsOffered) ? s.subjectsOffered : [];
    $("specificSubjectsWrap").style.display = isAll ? "none" : "block";
    // Load in background even if hidden — so checkboxes are ready when user unticks "All"
    await loadSubjectCheckboxes(s.classBase, selectedSubs);
  } else {
    $("specificSubjectsWrap").style.display = "none";
  }
  openModal("studentModal");
